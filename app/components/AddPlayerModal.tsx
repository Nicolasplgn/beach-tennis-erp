...
export function AddPlayerModal({ leagueId }: { leagueId: string }) {
  ...
  // Garante que o código rode apenas no navegador para usar o Portal
  useEffect(() => {
    setMounted(true)
    if (isOpen) {
  ...
  // O conteúdo do Modal agora é uma variável separada
  const modalContent = (
    // Z-INDEX 9999 e FIXED para garantir que fique em cima de tudo
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-md animate-in fade-in duration-200">
      <div 
        className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden relative animate-in zoom-in-95 duration-200 border border-slate-200"
  ...
  return (
    <>
      ...
      {/* O SEGREDO ESTÁ AQUI: PORTAL PARA O BODY */}
